...
/**
 * Normalize blog list payloads from various API shapes into a consistent array.
 * @param {Object|Array|null} response
 * @returns {Array<{id, title, image, description, created_at}>}
 */
export function normalizeBlogPosts(response) {
    if (!response) return [];
    ...
    return list.map((b) => {
        const rawImage = b.image || b.image_url || b.image_path || '/no-image.svg';
        return {
            id: b.id,
